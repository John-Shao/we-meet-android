...
class ImSession private constructor(deps: ImDeps, appContext: Context) {
    ...
    init {
        ...
        // in [mentionScan] (model/MentionAliases.kt) and nowhere else.
        scope.launch {
            client.messages.collect { m ->
                ...
                // 已知缺口:只认目录名,不认我在这个群里的**群昵称** —— Web 两个都
                // 认(它手上有缓存的 roster)。这边 ImSession 没有 roster,补它要另
                // 起一条数据线,不在本次范围。签名收 List 就是给那一天留的口子。
                val selfName = self?.let { userDirectory.get(it)?.displayName }
                val hit = mentionScan(m.contentType, m.body, listOf(selfName))
