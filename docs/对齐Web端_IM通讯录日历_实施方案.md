...
### 三次改动:拼音搜索（后端 `0145`，App 零逻辑改动）

> 依赖 we-meet 后端的 `0145`(`User.search_key` = 全拼 + 首字母缩写)。App 的搜索
> 一直把 `q` 发给 `/directory/members/?q=`,所以**一行逻辑都不用改** —— 只把占位文案
> 从「搜索姓名或邮箱」改成「搜索姓名、拼音或邮箱」(5 语言)。输入 `ye` / `ylx` / `夜`
> 现在都能命中「夜来香」。该能力无语言门槛:拼音是写出来的,任何界面语言都能敲。

### 四次改动:通讯录首页改成入口式（对标飞书）

首页原来把「固定入口 + 部门树 + 全部成员名单」堆在同一屏 —— 一屏里既有导航又有内容,
而部门树还只是组织层级的第一层。现在拆成两层:

- **通讯录 tab(一级页)= 入口**:内部联系人 / 外部联系人 / 星标联系人 / 我的群组,
  分成三组,组间用灰缝分开(飞书的卡片分组)。
- 顶部与飞书对齐:**搜索入口是工具栏右侧那个放大镜**(页面里不再摆搜索框 —— 它占掉
  一整行,还把组织信息挤下去);工具栏下面第一行是**当前组织**(头像 + 名字,来自
  `GET /directory/me/`;取不到就不显示这一行,它不是页面的骨架)。除这一次单行查询外
  首页没有 ViewModel、也没有列表请求。
...
- `ContactsViewModel` 的作用域随之从「tab 的 HOME 条目」变成 `org_contacts` 路由条目:
  退出那一页下钻状态即清掉,下次进来从组织根开始(路由级作用域的自然语义)。
