...
@Composable
internal fun rememberTranscriptExporter(
    repository: MeetingRecordRepository,
    viewer: String,
    recordId: String,
): (String) -> Unit = key(viewer, recordId) {
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        ...
        if (result.resultCode != Activity.RESULT_OK || selectedFormat == null || destination == null) return@rememberLauncherForActivityResult
        busy = true
        scope.launch {
            val written = runCatching {
                withContext(Dispatchers.IO) {
                    // Reauthorize on return; never retain transcript bytes in the picker.
                    repository.transcriptExport(viewer, recordId, selectedFormat).getOrThrow().use { body ->
    ...
    val launch: (String) -> Unit = { selectedFormat ->
        ...
        if (!busy && format == null) {
            ...
            picker.launch(Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
                ...
                type = mime
                putExtra(Intent.EXTRA_TITLE, "transcript-${recordId.take(8)}.$selectedFormat")
            })
        }
    }
    launch
